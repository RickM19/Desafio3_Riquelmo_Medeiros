...
export default class UpdateOrderService {
    public async execute({
        ...
        if (DataInicial.getTime() < today.getTime()) {
            throw new AppError(
                `${DataInicial} essa data é menor que ${today}`,
                400,
            );
        }
        ...
        if (DataFinal < DataInicial) {
            throw new AppError(
                `${DataFinal} essa data é menor que ${DataInicial}`,
                400,
            );
        }
        // update do CEP
        ...
        await order.update({
            ...
            status,
            DataCancelamento:
                status === 'Cancelado' ? order.DataCancelamento : null,
        });
